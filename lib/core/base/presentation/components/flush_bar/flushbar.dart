...
import 'package:nl_web/core/base/presentation/components/flush_bar/flushbar_route.dart'
    as route;
...
class Flushbar<T> extends StatefulWidget {
  Flushbar(
      ...
      this.padding = const EdgeInsets.all(AppDimens.mPaddingMedium),
      this.borderRadius,
...
class _FlushbarState<K extends Object?> extends State<Flushbar<K>>
  ...
  bool get _isFlag => widget.flagType != null;

  Widget _generateFlushbar() {
    Color backgroundColor =
        widget.flagType?.backgroundColorByType() ?? widget.backgroundColor;
/*    Color textColor = widget.flagType!.contentColorByType();
    Color buttonColor = widget.flagType!.buttonColorByType();
    Widget iconColor = widget.flagType!.iconByType();*/
    ...
    return Container(
      ...
      constraints: widget.maxWidth != null
          ? BoxConstraints(maxWidth: widget.maxWidth!)
          : null,
      padding: EdgeInsets.all(AppDimens.mPaddingMedium),
      decoration: BoxDecoration(
        color: backgroundColor,
        gradient: widget.backgroundGradient,
        boxShadow: widget.boxShadows,
  ...
  List<Widget> _getHeaderPanel() {
    ...
    return <Widget>[
      ...
      Expanded(
        flex: 1,
        child: Container(
          margin: EdgeInsets.only(top: AppDimens.mSpaceXSmall2),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              (isExpanded || widget.flagType == FlagType.normal)
                  ? Padding(
                      padding: EdgeInsets.only(
                        top: 0,
                        left: 4.0,
                  ...
                  : Padding(
                      ...
                      child: Text(
                        widget.titleHeader ?? '',
                        style: AppStyles.mSTH500.copyWith(
                          fontSize: widget.titleSize ?? AppDimens.m500,
                          color: widget.titleColor ?? AppColor.mCWhite,
                        ),
                      ),
                    ),
              if (isExpanded || widget.flagType == FlagType.normal)
                Padding(
                  padding: EdgeInsets.only(
                    top: _messageTopMargin,
                    left: 4.0,
      ...
      widget.flagType != FlagType.normal
          ? Padding(
              padding: EdgeInsets.only(right: buttonRightPadding),
              child: IconButton(
                icon: Icon(
                  isExpanded ? Icons.arrow_drop_up : Icons.arrow_drop_down,
                  color: widget.flagType!.contentColorByType(),
                  size: AppDimens.mIconNormal,
                ),
                iconSize: AppDimens.mIconNormal,
  ...
  Widget _buildAction() {
    if (widget.positiveActionText != null &&
        widget.negativeActionText != null) {
      return Row(
        mainAxisAlignment: MainAxisAlignment.start,
        children: [
          TextButton(
            child: Text(widget.positiveActionText!,
                style: AppStyles.mSTBaseLineNormal.copyWith(
                    color: _isFlag && widget.flagType != FlagType.normal
                        ? widget.flagType!.contentColorByType()
                        : AppColor.mCBlue400)),
            style: ButtonStyle(
              backgroundColor: MaterialStateProperty.all(
                  _isFlag ? widget.flagType!.buttonColorByType() : null),
            ),
          ...
          TextButton(
            child: Text(widget.negativeActionText!,
                style: AppStyles.mSTBaseLineNormal.copyWith(
                    color: _isFlag && widget.flagType != FlagType.normal
                        ? widget.flagType!.contentColorByType()
                        : AppColor.mCBlue400)),
